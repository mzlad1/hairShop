...
class CacheManager {
  ...
  static set(key, data, ttl) {
    try {
      ...
      console.log(`Cache set for ${key} with TTL: ${finalTTL}ms`);
    } catch (error) {
  ...
  static get(key) {
    try {
      ...
      if (age > parsed.ttl) {
        console.log(
          `Cache expired for ${key} (age: ${age}ms, ttl: ${parsed.ttl}ms)`
        );
        localStorage.removeItem(key);
        return null;
      }

      console.log(`Cache hit for ${key} (age: ${age}ms, ttl: ${parsed.ttl}ms)`);
      return parsed.data;
    } catch (error) {
